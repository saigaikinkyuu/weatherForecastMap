    ...
    for(var i = 0;i<data.length + 12;i++){
      if(i>(data.length-1)){
        if(Number((data[0].basetime).slice(10,12)) + 5*(i - (data.length-1)) > 55){
          let baseTime = (data[0].basetime).slice(0,8) + "" + ("0" + (hour_json+1)).slice(-2) + (Number((data[0].basetime).slice(10,12)) + 5*(i - (data.length-1)) -60) + "00"
          rcArray.unshift([baseTime,baseTime])
        }else {
          let baseTime = (data[0].basetime).slice(0,8) + "" + ("0" + hour_json).slice(-2) + Number((data[0].basetime).slice(10,12)) + 5*(i - (data.length-1)) + "00"
          rcArray.unshift([baseTime,baseTime])
        }
      }else {
