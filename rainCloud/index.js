...
function dateSend(){
  ...
  const interval = setInterval(function() {
    if(stopFlag === true){
      ...
      document.getElementById("date").textContent = (data[0]).slice(0,4) + "年" + (data[0]).slice(4,6) + "月" + (data[0]).slice(6,8) + "日 " + "<input id='inputTime1' class='timeInput' type='number' value='" + ("0" + hour_json).slice(-2) + "'>" + "時" + "<input id='inputTime2' class='timeInput' type='number' value='" + (data[0]).slice(10,12) + "'>分"
