...
$.getJSON("https://www.jma.go.jp/bosai/himawari/data/satimg/targetTimes_jp.json", function (data) {
    ...
    var baseTime3 = data[n].basetime;
    var validTime3 = data[n].validtime;
    var hour_json = Number((data[n].basetime).slice(8,10)) + 9
    if(hour_json > 23){
      hour_json = hour_json - 24
    }
    document.getElementById("date_HIMAWARI").textContent = "［ひまわり］" + (data[n].basetime).slice(0,4) + "年" + (data[n].basetime).slice(4,6) + "月" + (data[n].basetime).slice(6,8) + "日 " + ("0" + hour_json).slice(-2) + "時" + (data[n].basetime).slice(10,12) + "分"
